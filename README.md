...
### 3. 🔨 Gores 模式状态机事务重构 (任意武器出锤敲击自恢复)
* **状态机事务架构**：重构了底层的输入流，开火时自动进入事务流：`记录当前武器 -> 切锤子 -> 检测拿稳锤子后瞬间开火挥击 -> 自动切回原手持武器`。
* **支持任意原武器**：无论手持**激光、榴弹、霰弹还是手枪**，按下开火都能瞬间敲出一锤并无缝恢复刚才手持的武器，彻底杜绝打空与误射。
* **事务锁保护**：出锤事务执行期间自动加锁，避免连点或网络抖动导致切枪错乱。
* **快捷按键开关**：控制台命令 `toggle_gores_mode`，可在 **「设置」 -> 「扩展」 -> 「Gores Mode」** 直接录制开关按键。
...
### 6. ⏱️ 定时任务调度器 (Task Scheduler / 宏与定时器)
* **可视化操作**：进入 **「设置」 -> 「扩展」 -> 「定时任务调度器」** 即可直接输入指令、设定毫秒延时并创建。
* **两种调度模式**：
  * **单次延迟 (Timeout)**：例如 `timeout 3000 "say 3秒后就绪"`，在指定毫秒后执行一次。
  * **循环定时 (Interval)**：例如 `interval 5000 "say [AFK] 挂机中"`，每隔指定毫秒周期性重复执行。
* **控制台 CLI 命令**：
  * `timeout <毫秒> <指令>`：创建单次延时任务。
  * `interval <毫秒> <指令>`：创建循环定时任务。
  * `stop_tasks [任务ID]`：停止指定编号的任务，或不带参数一键清空所有任务。
  * `list_tasks`：查看当前正在运行的定时任务列表。
...
| 指令 | 默认参数/类型 | 说明 |
| :--- | :--- | :--- |
| `timeout <ms> <cmd>` | 指令 | 单次延迟执行控制台指令 |
| `interval <ms> <cmd>` | 指令 | 周期性循环执行控制台指令 |
| `stop_tasks [id]` | 指令 | 停止指定任务或一键清空所有定时任务 |
| `list_tasks` | 指令 | 列出所有活跃的定时任务 |
